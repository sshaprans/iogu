...
// Ініціалізуємо масив налаштувань
$configGroups = [];
...
try {
    ...
    // Таблиця settings має структуру: id, key_name, value_uk, value_en, group_name
    $stmt = $db->query("SELECT * FROM settings");
    $settings = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Визначаємо поточну мову
    $currentLang = 'uk';
    if (function_exists('is_lang_active') && is_lang_active('en')) {
        $currentLang = 'en';
    } elseif (isset($_GET['lang']) && $_GET['lang'] === 'en') {
        $currentLang = 'en';
    }

    $valKey = 'value_' . $currentLang; // value_uk або value_en

    foreach ($settings as $item) {
        // Якщо group_name пустий, кладемо в 'general'
        $group = !empty($item['group_name']) ? $item['group_name'] : 'general';
        $key = $item['key_name'];
        // Беремо значення для мови, або фолбек на укр
        $value = !empty($item[$valKey]) ? $item[$valKey] : ($item['value_uk'] ?? '');

        $configGroups[$group][$key] = $value;
    }

} catch (Exception $e) {
    // Тихо ігноруємо помилки, масив залишиться пустим, спрацюють ?? дефолтні значення
}
?>
    ...
    <div class="header-wrap">
        <div class="header-up">
            ...
            <div class="header-up__ui-wrapper">
                ...
                <!-- ТУТ ТЕЖ МОЖНА ВИКОРИСТАТИ НАЛАШТУВАННЯ ДЛЯ ТЕЛЕФОНУ -->
                <a href="tel:<?= preg_replace('/[^\d+]/', '', $configGroups['general']['phone'] ?? '0443565321') ?>" class="header-up__ui--tel header-link">
                    <?= $configGroups['general']['phone'] ?? '(044)-356-53-21' ?>
                </a>

                <ul class="header-up__social-list">
                    <!-- FB -->
                    <li class="header-up__social-item">
                        <a href="<?= $configGroups['social']['social_facebook'] ?? 'https://www.facebook.com/iogu.gov.ua' ?>" target="_blank" class="header-up__social-link">
                    ...
                    <li class="header-up__social-item">
                        <a href="<?= $configGroups['social']['social_telegram'] ?? 'https://t.me/+380930138074' ?>" target="_blank" class="header-up__social-link">
                            <svg class="header-social" width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <li class="header-up__social-item">
                        <a href="<?= $configGroups['social']['social_viber'] ?? 'viber://chat?number=%2B380930138074' ?>" target="_blank" class="header-up__social-link">
                            <svg class="header-social" width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <li class="header-up__social-item">
                        <a href="<?= $configGroups['social']['social_whatsapp'] ?? 'https://wa.me/380930138074' ?>" target="_blank" class="header-up__social-link">
                            <svg class="header-social" width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
