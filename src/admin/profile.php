...
<?php if ($message): ?>
    <div style="background:#d5f5e3; color:#27ae60; padding:15px; border-radius:4px; margin-bottom:20px;">
        <?= $message ?>
    </div>
<?php endif; ?>

<?php if ($error): ?>
    <div style="background:#fadbd8; color:#c0392b; padding:15px; border-radius:4px; margin-bottom:20px;">
        <?= $error ?>
    </div>
<?php endif; ?>
    ...
    <form class="form-card" method="POST" enctype="multipart/form-data" style="max-width: 600px;">
        ...
        <div class="form-group">
            <label class="form-label">Логін (не змінюється)</label>
            <input type="text" class="form-control" value="<?= htmlspecialchars($userData['login']) ?>" disabled style="background:#eee; color: #777;">
        </div>
        ...
        <div class="form-group">
            ...
            <input type="password" name="new_password" class="form-control" placeholder="Введіть новий пароль">
        </div>
        ...
        <div class="form-group">
            ...
            <div class="hint">Заповнюйте ці поля, тільки якщо хочете змінити пароль. Мінімум 6 символів.</div>
        </div>
